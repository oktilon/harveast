...
class ProcessingType {
    ...
    public function __construct($arg = 0) {
        ...
        if($fld) {
            $q = $DB->prepare("SELECT * FROM processing_types WHERE $fld = :v")
    ...
    public static function init($name) {
        if(empty(trim($name))) return new ProcessingType();
        $ret = new ProcessingType($name);
    ...
    public function save() {
        $t = new SqlTable('processing_types', $this);
        return $t->save($this);
    }
    ...
    public static function get($id) {
        if(!isset(self::$cache[$id])) {
            self::$cache[$id] = new ProcessingType($id);
        }
    ...
    public static function getList($flt = [], $ord = 'name', $lim = '') {
        ...
        $DB->prepare("SELECT $calc $flds FROM processing_types $add $order $limit");
        foreach($par as $k => $v) {
            $DB->bind($k, $v);
        }
        $rows = $DB->execute_all();
        ...
        foreach($rows as $row) {
            $ret[] = $flds == '*' ? new ProcessingType($row) : ($flds == 'id' ? intval($row['id']) : $row);
        }
        self::$total = $total;
        return $ret;
    }
