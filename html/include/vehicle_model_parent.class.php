...
class VehicleModelParent {
    ...
    public function __construct($arg = 0) {
        ...
        if($fld) {
            $q = $DB->prepare("SELECT * FROM vehicle_model_parents WHERE $fld = :v")
    ...
    public static function init($name) {
        if(empty(trim($name))) return new VehicleModelParent();
        $ret = new VehicleModelParent($name);
    ...
    public function save() {
        $t = new SqlTable('vehicle_model_parents', $this);
        return $t->save($this);
    }
    ...
    public static function get($id) {
        if(!isset(self::$cache[$id])) {
            self::$cache[$id] = new VehicleModelParent($id);
        }
    ...
    public static function getList($flt = [], $ord = 'name', $lim = '') {
        ...
        $DB->prepare("SELECT $calc $flds FROM vehicle_model_parents $add $order $limit");
        foreach($par as $k => $v) {
            $DB->bind($k, $v);
        }
        $rows = $DB->execute_all();
        ...
        foreach($rows as $row) {
            $ret[] = $flds == '*' ? new VehicleModelParent($row) : ($flds == 'id' ? intval($row['id']) : $row);
        }
        self::$total = $total;
        return $ret;
    }
