...
class ProductionRateParent {
    ...
    public function __construct($arg = 0) {
        ...
        if($fld) {
            $q = $DB->prepare("SELECT * FROM production_rate_parents WHERE $fld = :v")
    ...
    public static function init($name) {
        if(empty(trim($name))) return new ProductionRateParent();
        $ret = new ProductionRateParent($name);
    ...
    public function save() {
        $t = new SqlTable('production_rate_parents', $this);
        return $t->save($this);
    }
    ...
    public static function get($id) {
        if(!isset(self::$cache[$id])) {
            self::$cache[$id] = new ProductionRateParent($id);
        }
    ...
    public static function getList($flt = [], $ord = 'name', $lim = '') {
        ...
        $DB->prepare("SELECT $calc $flds FROM production_rate_parents $add $order $limit");
        foreach($par as $k => $v) {
            $DB->bind($k, $v);
        }
        $rows = $DB->execute_all();
        ...
        foreach($rows as $row) {
            $ret[] = $flds == '*' ? new ProductionRateParent($row) : ($flds == 'id' ? intval($row['id']) : $row);
        }
        self::$total = $total;
        return $ret;
    }
