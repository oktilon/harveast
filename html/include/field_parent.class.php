...
class FieldParent {
    ...
    public function __construct($arg = 0) {
        ...
        if($fld) {
            $q = $DB->prepare("SELECT * FROM field_parents WHERE $fld = :v")
    ...
    public static function init($name) {
        if(empty(trim($name))) return new FieldParent();
        $ret = new FieldParent($name);
    ...
    public function save() {
        $t = new SqlTable('field_parents', $this);
        $ret = $t->save($this);
    }
    ...
    public static function get($id) {
        if(!isset(self::$cache[$id])) {
            self::$cache[$id] = new FieldParent($id);
        }
    ...
    public static function getList($flt = [], $ord = 'name', $lim = '') {
        ...
        $DB->prepare("SELECT $calc $flds FROM field_parents $add $order $limit");
        foreach($par as $k => $v) {
            $DB->bind($k, $v);
        }
        $rows = $DB->execute_all();
        ...
        foreach($rows as $row) {
            $ret[] = $flds == '*' ? new FieldParent($row) : ($flds == 'id' ? intval($row['id']) : $row);
        }
        self::$total = $total;
        return $ret;
    }
